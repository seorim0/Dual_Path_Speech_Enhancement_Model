...
if opt.arch == 'DPCNU':
    chkpt_main = torch.load('./main.pt', map_location='cuda:0')
    chkpt_auxiliary = torch.load('./main.pt', map_location='cuda:0')
    model.main_path.load_state_dict(chkpt_main['model'])
    model.auxiliary_path.load_state_dict(chkpt_auxiliary['model'])
...
with torch.no_grad():
    for inputs, targets, input_addrs in utils.Bar(test_loader):
        data_num += 1
        ...
        if opt.wav_write_flag:
            save_name = str(input_addrs[0])[-12:]
            write('./results/{}/{}'.format(opt.test_name, save_name), opt.fs, enhanced_wavs[0])
